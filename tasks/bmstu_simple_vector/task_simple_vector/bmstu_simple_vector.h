...
#pragma endregion
    ...
    simple_vector(std::initializer_list<T> init) : size_(init.size()), capacity_(init.size()) {
        data_.reset(new T[capacity_]);
        std::copy(init.begin(), init.end(), data_.get());
    }

    simple_vector(const simple_vector& other) : size_(other.size_), capacity_(other.capacity_) {
        data_.reset(new T[capacity_]);
    ...
    simple_vector(size_t size, const T& value = T{}) : size_(size), capacity_(size) {
        data_.reset(new T[capacity_]);
